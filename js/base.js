	...
	return class QrCodeRecognition {
		...
		sweep() {
			...
			if (navigator.mediaDevices) {
				...
				if (navigator.userAgent.toLowerCase().match(/chrome/) && location.origin.indexOf(
						'https://') < 0) {
		...
		upload() {
			...
			const createObjectURL = window.createObjectURL || window.URL.createObjectURL || window
				.webkitURL.createObjectUR;
